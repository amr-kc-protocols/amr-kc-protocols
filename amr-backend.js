...
(function (global) {
  ...
  /* ── session ───────────────────────────────────────────────
     One anonymous user per device, created silently on first use.
     The learner can attach an email later (linkEmail) to keep the
  ...
  function refreshSession(stored) {
    ...
    refreshInFlight = request("/auth/v1/token?grant_type=refresh_token", {
      ...
      if (res.ok && res.data && res.data.access_token) return saveSession(res.data);
  ...
  function authed(path, options, session) {
    ...
    return request(path, opts).then(function (res) {
      ...
      return refreshSession(stored).then(function (fresh) {
        if (!fresh) return res;
